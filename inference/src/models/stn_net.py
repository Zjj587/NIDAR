...
class Decoder(nn.Module):
    """EDNet 解码器块"""
    def __init__(self, out_shape, in_channels, out_channels, final_out_channels,
                 skip_channels, out_scale=0, ksize=3):
        super(Decoder, self).__init__()
        self.out_shape = out_shape
        self.conv = Conv2dAP(out_shape, out_shape, in_channels, out_channels, ksize, 1, False)
        self.convbn = nn.BatchNorm2d(out_channels)
        self.iconv = Conv2dAP(out_shape, out_shape, out_channels + skip_channels,
                             out_channels, ksize, 1, False)
        self.iconvbn = nn.BatchNorm2d(out_channels)
        if out_scale > 0:
            self.out = Conv2dAP(out_shape, out_shape, out_channels,
                               final_out_channels, ksize, 1, True)
...
class STN(nn.Module):
    """Spectral Translation Network - RGB到伪NIR的转换网络"""
    ...
    def forward(self, rgbs, exp_ratio):
        """
        Args:
            rgbs: RGB图像的多尺度金字塔列表 [im0, im1, im2, im3]
            exp_ratio: 曝光比率,控制伪NIR的亮度
        Returns:
            transes: 伪NIR图像的多尺度金字塔列表
        """
        if self.filt:
